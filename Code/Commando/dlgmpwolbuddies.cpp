...
void MPWolBuddiesMenuClass::Update_Buddy_Ranking(int index, const RefPtr<WWOnline::UserData>& user)
{
	...
	if (list) {
		...
		if (ladder.IsValid()) {
			...
			swprintf(text, _countof(text), L"%d", ladder->GetWins());
			list->Set_Entry_Text(index, COL_WINS, text);

			swprintf(text, _countof(text), L"%d / %d", ladder->GetReserved1(), ladder->GetKills());
			list->Set_Entry_Text(index, COL_DEATHS, text);

			swprintf(text, _countof(text), L"%d", ladder->GetPoints());
			list->Set_Entry_Text(index, COL_POINTS, text);

			swprintf(text, _countof(text), L"%d", ladder->GetRung());
