...
DECLARE_SCRIPT(GTH_Zone_Send_Custom, "Enter_Message=20000:int,Enter_Param=1:int,Exit_Message=20000:int,Exit_Param=0:int")
{
	void Entered( GameObject * obj, GameObject * enterer )
	{
		...
		if (message != 0) {
			Commands->Send_Custom_Event(obj,enterer,message,Get_Int_Parameter("Enter_Param"), 0);
		}
	}
	...
	void Exited( GameObject * obj, GameObject * exiter ) 
	{
		...
		if (message != 0) {
			Commands->Send_Custom_Event(obj,exiter,message,Get_Int_Parameter("Exit_Param"), 0);
		}
	}
};
